...
#
# Behaviors:
# - Auto-detects tag from debian/changelog (e.g. 0.1.0 -> v0.1.0) if not provided
...
#
# Usage examples:
#   scripts/utils/upload-image.sh --tag v0.1.0 out/vm-0.1.0.qcow2
...
Options:
  --tag TAG       Release tag (e.g. v0.1.0). If omitted, parsed from debian/changelog as v<version>.
...
parse_tag_from_debian_changelog() {
  ...
  # Expect first line like: "servobox (0.1.0) unstable; urgency=medium"
  local first_line
